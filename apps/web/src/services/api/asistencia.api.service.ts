...
interface ApiDetalleResponse {
  items: AsistenciaDetalleResponse['items'];
  page: number;
  pageSize: number;
  total: number;
  totalPages: number;
  ...
  implements AsistenciaService
{
  async getResumen(
    ...
    return fetchJson<AsistenciaResumen>(
      `/bi/asistencia/resumen?${buildQuery(filters)}`,
    );
  }
  ...
  async getDetalle(
    ...
    const response =
      await fetchJson<ApiDetalleResponse>(
        `/bi/asistencia/detalle?${buildQuery(
          filters,
          true,
        )}`,
      );
...
export const asistenciaApiService =
  new AsistenciaApiService();
