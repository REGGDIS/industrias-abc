...
export interface ContratosCatalogos {
  anios: SelectOption[];
  meses: SelectOption[];
  areas: SelectOption[];
...
async function getApiCatalogos(
  ...
  const [
    ...
    fetchJson<PeriodosApiResponse>(
      '/bi/catalogos/periodos?dominio=contratos',
    ),
    fetchJson<ItemsResponse>(
      '/bi/catalogos/areas',
    ),
    fetchJson<ItemsResponse>(
      `/bi/contratos/trabajadores?${buildTrabajadoresQuery(
        filters,
      )}`,
    ),
  ]);
...
function getMockCatalogos(): ContratosCatalogos {
  return {
    anios: aniosMock,
    meses: mesesMock,
    areas: areasMock,
...
export async function getContratosCatalogos(
  filters: BiFilters,
): Promise<ContratosCatalogos> {
