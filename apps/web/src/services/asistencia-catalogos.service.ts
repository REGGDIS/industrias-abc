...
export interface AsistenciaCatalogos {
  anios: SelectOption[];
  meses: SelectOption[];
  areas: SelectOption[];
...
async function getApiCatalogos(
  ...
  const [
    ...
    fetchJson<PeriodosApiResponse>(
      '/bi/catalogos/periodos?dominio=asistencia',
    ),
    fetchJson<ItemsResponse>(
      '/bi/catalogos/areas',
    ),
    fetchJson<ItemsResponse>(
      `/bi/asistencia/trabajadores?${buildTrabajadoresQuery(
        filters,
      )}`,
    ),
  ]);
...
function getMockCatalogos(): AsistenciaCatalogos {
  return {
    anios: aniosMock,
    meses: mesesMock,
    areas: areasMock,
...
export async function getAsistenciaCatalogos(
  filters: BiFilters,
): Promise<AsistenciaCatalogos> {
