/* ============================================================================
   ...
   ----------------------------------------------------------------------------
   Contenido:
   1) Empleado (referencia local, basada en datos de referencia del Universo
      Empresarial). La gestión operacional del trabajador corresponde al
      sistema RRHH; esta referencia local existe para poder poblar Contrato y
...
-- ----------------------------------------------------------------------------
-- 1) Empleado (referencia local basada en datos de referencia del Universo Empresarial)
-- ----------------------------------------------------------------------------
...
INSERT INTO dbo.Empleado (empleado_id, rut_referencia, nombre_completo, codigo_area_ref, codigo_cargo_ref, fecha_ingreso_ref) VALUES (N'EMP080', N'15077719-4', N'Catalina Henríquez Medina', N'A07', N'C10', '2025-12-17');
GO
...
-- Nuevo tipo APORTE: aportes del empleador que no corresponden a un haber
-- del trabajador ni a un descuento (p. ej. seguro complementario pagado por
-- la empresa). Se deja el catálogo preparado; no se fuerza su uso masivo.
(N'APORTE_SEGURO', N'Aporte empresa seguro complementario', 'APORTE', 0);
GO

-- ----------------------------------------------------------------------------
-- 3a) Contrato + Liquidacion + Detalle — registro de ejemplo (EMP001)
-- ----------------------------------------------------------------------------
-- Corregido: sueldo_base original (850000) era muy discordante frente al cargo
-- de referencia "Gerente General" (inferior incluso al de varios Analistas).
-- Se ajusta a un valor coherente con la jerarquía del cargo; la liquidación y
-- su detalle se recalculan en consecuencia.
...
VALUES (N'EMP001', N'CT-2024-0001', 'INDEFINIDO', '2019-06-07', NULL, 'COMPLETA', 3800000, N'Gerente General', 'VIGENTE');
...
VALUES (N'EMP001', @contrato_id, '2026-07', 3800000, 4, 4247000, 3505000, 4450000);
...
FROM (VALUES
    (N'HORAS_EXTRA',  157000),
    (N'GRATIF_LEGAL', 290000),
    (N'COLACION',     45000),
...
-- ----------------------------------------------------------------------------
-- 3b) Contrato + Liquidacion + Detalle — 20 empleados adicionales de referencia
-- ----------------------------------------------------------------------------
-- Contratos adicionales (20 empleados tomados de los datos de referencia del Universo Empresarial)
...
-- Corregido: todo contrato TEMPORAL debe registrar fecha_termino (CK_Contrato_fecha_termino).
-- Se asigna una fecha_termino coherente y posterior a fecha_inicio, y posterior también
-- al período de la liquidación de ejemplo (2026-07) ya que el contrato figura VIGENTE.
INSERT INTO dbo.Contrato (empleado_id, numero_contrato, tipo_contrato, fecha_inicio, fecha_termino, jornada, sueldo_base, cargo_contrato, estado) VALUES (N'EMP021', N'CT-2024-0021', 'TEMPORAL', '2022-01-08', '2026-12-31', 'COMPLETA', 1013000, N'Asistente Administrativo', 'VIGENTE');
