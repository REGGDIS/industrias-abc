/* ============================================================================
   ...
   Basado en:      Modelo Conceptual v0.1 (aprobado) e IDF v0.1 (aprobado)
   ...
   DetalleLiquidación, más una tabla local de referencia Empleado (copia de
   trabajo del Universo Empresarial Master v0.2; el maestro real de personal
   pertenece al sistema de RRHH y NO se administra aquí).
   ============================================================================ */
...
--    No es dueña de este dominio. Es una copia de trabajo del Universo
--    Empresarial Master v0.2 (hoja Empleados), usada solo para poder construir
...
-- 2. Contrato
-- ----------------------------------------------------------------------------
IF OBJECT_ID(N'dbo.Contrato', N'U') IS NOT NULL DROP TABLE dbo.Contrato;
GO
...
-- ----------------------------------------------------------------------------
IF OBJECT_ID(N'dbo.ConceptoPago', N'U') IS NOT NULL DROP TABLE dbo.ConceptoPago;
GO
CREATE TABLE dbo.ConceptoPago (
    ...
    CONSTRAINT CK_ConceptoPago_tipo CHECK (tipo IN ('HABER','DESCUENTO'))
);
GO
...
-- 4. Liquidacion
-- ----------------------------------------------------------------------------
IF OBJECT_ID(N'dbo.Liquidacion', N'U') IS NOT NULL DROP TABLE dbo.Liquidacion;
GO
CREATE TABLE dbo.Liquidacion (
    ...
    CONSTRAINT CK_Liquidacion_periodo CHECK (periodo LIKE '[0-9][0-9][0-9][0-9]-[0-9][0-9]'),
...
-- ----------------------------------------------------------------------------
IF OBJECT_ID(N'dbo.DetalleLiquidacion', N'U') IS NOT NULL DROP TABLE dbo.DetalleLiquidacion;
GO
CREATE TABLE dbo.DetalleLiquidacion (
    ...
    CONSTRAINT FK_Detalle_Liquidacion FOREIGN KEY (liquidacion_id)
        REFERENCES dbo.Liquidacion (liquidacion_id) ON DELETE CASCADE,
