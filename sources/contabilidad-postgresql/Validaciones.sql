...
-- -----------------------------------------------------------------------------
-- 3. DETECCIÓN DE NULOS EN CAMPOS OBLIGATORIOS (Deben retornar 0 filas)
-- -----------------------------------------------------------------------------
SELECT 'areas_nulos' AS prueba, COUNT(*) AS fallos 
FROM areas 
WHERE area_id IS NULL OR codigo_area IS NULL OR nombre_area IS NULL
UNION ALL
SELECT 'centros_costo_nulos', COUNT(*) 
FROM centros_costo 
WHERE centro_costo_id IS NULL OR codigo IS NULL OR nombre IS NULL OR area_id IS NULL OR estado IS NULL
UNION ALL
SELECT 'cuentas_contables_nulos', COUNT(*) 
FROM cuentas_contables 
WHERE cuenta_id IS NULL OR codigo_cuenta IS NULL OR nombre_cuenta IS NULL OR tipo_cuenta IS NULL OR grupo IS NULL OR nivel IS NULL OR estado IS NULL
UNION ALL
SELECT 'movimientos_contables_nulos', COUNT(*) 
FROM movimientos_contables 
WHERE movimiento_id IS NULL OR fecha IS NULL OR cuenta_id IS NULL OR centro_costo_id IS NULL OR documento_tipo IS NULL OR documento_numero IS NULL OR debe IS NULL OR haber IS NULL OR tipo_cambio IS NULL;
...
LEFT JOIN cuentas_contables c ON m.cuenta_id = c.cuenta_id 
...
FROM movimientos_contables m 
LEFT JOIN centros_costo cc ON m.centro_costo_id = cc.centro_costo_id 
WHERE cc.centro_costo_id IS NULL;

-- 4.3 Validación de relación 1:1 entre Áreas y Centros de Costo (Huérfanos y Unicidad)
SELECT 
    a.codigo_area,
    a.nombre_area,
    ...
    cc.estado
FROM areas a
LEFT JOIN centros_costo cc ON a.area_id = cc.area_id;

-- -----------------------------------------------------------------------------
-- 5. REGLAS DE NEGOCIO: PLAN DE CUENTAS
-- -----------------------------------------------------------------------------
-- 5.1 Cuentas hijas con cuenta_padre_id inexistente (Huérfanas en jerarquía)
SELECT c.cuenta_id, c.codigo_cuenta, c.cuenta_padre_id 
FROM cuentas_contables c
WHERE c.cuenta_padre_id IS NOT NULL 
  AND c.cuenta_padre_id NOT IN (SELECT cuenta_id FROM cuentas_contables);
...
SELECT 
    ...
    c.tipo_cuenta,
    c.estado,
    COALESCE(p.codigo_cuenta || ' - ' || p.nombre_cuenta, '(RAÍZ)') AS cuenta_padre
FROM cuentas_contables c
LEFT JOIN cuentas_contables p ON c.cuenta_padre_id = p.cuenta_id
ORDER BY c.codigo_cuenta;

-- -----------------------------------------------------------------------------
-- 6. REGLAS DE NEGOCIO: MOVIMIENTOS CONTABLES (DEBE / HABER / TIPO CAMBIO)
-- -----------------------------------------------------------------------------
-- 6.1 Movimientos con montos negativos, ambos en cero o ambos positivos (Debe dar 0 filas)
SELECT * 
FROM movimientos_contables
WHERE debe < 0 
   OR haber < 0 
...
-- 6.2 Cuadratura contable general (Balance de Comprobación: Total Debe vs Total Haber)
SELECT 
...
FROM cuentas_contables c
JOIN movimientos_contables m ON c.cuenta_id = m.cuenta_id
GROUP BY c.tipo_cuenta, c.grupo
ORDER BY c.tipo_cuenta, c.grupo;
...
LEFT JOIN movimientos_contables m ON cc.centro_costo_id = m.centro_costo_id
